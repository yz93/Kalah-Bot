...
public:
	...
	bool move();
	//If the game is over, return false.Otherwise, make a complete move for the player whose turn it is(so that it becomes 
	//the other player's turn) and return true. "Complete" means that the player sows the seeds from a hole and takes any 
	//additional turns required or completes a capture. If the player gets an additional turn, you should display the board 
	//so someone looking at the screen can follow what's happening.
	void play();
	//Play the game. Display the progress of the game in a manner of your choosing, provided that someone looking at the 
	//screen can follow what's happening. If neither player is interactive, then to keep the display from quickly scrolling through 
